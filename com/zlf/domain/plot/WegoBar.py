...
class WegoBar(object):
    ...
    def xAxisPlot(self,dwg,x,y,y1,w,_list):
        ...
        for i in range(0,len(lines),2):
            ...
            dwg.add(dwg.text(txts[i/2], insert=((lines[i][0]+lines[i+1][0])/2, lines[i][1]+20),text_anchor='middle',font_size=10))
    ...
    def yAxis1Plot(self,dwg,x,y1,y2,_max=100):
        ...
        for i in range(5):
            ...
            dwg.add(dwg.text(str(int(_max-sp*i))+'%', insert=(x-5, y1+span*i+5),text_anchor='end',font_size=8)) # settings are valid for all text added to 'g'
        dwg.add(self.yLabelPlot(dwg, 'Percent of genes',x-(len(str(int(_max)))+1)*7-10,(y2+y1)/2))
    ...
    def yAxis2Plot(self,dwg,x,y1,y2,maxs):
        ...
        for i in range(5):
            dwg.add(dwg.line((x+5, y1+span*i), (x, y1+ span*i), stroke=svgwrite.rgb(10, 10, 16, '%')))
            if len(maxs)==1:
                dwg.add(dwg.text(str(int(maxs[0]*(1-0.25*i))), insert=(x+5, y1+span*i+5),font_size=8))
            elif len(maxs)==2:
        ...
        dwg.add(self.yLabelPlot(dwg, 'Number of genes',x+10+len(str(max(maxs)))*7,(y2+y1)/2))
            
    def yLabelPlot(self,dwg,label,x,y):
        txt=dwg.text(label, insert=(x,y),font_family='Times New Roman',font_size=8,text_anchor='middle')
        txt.rotate(-90,(x, y))
        return txt
...
def test3():
    ...
    mf=getLevelData(folder+'/WT-14d_vs_WT-0d/DN_MF_level2_classFun.txt')
    cc=getLevelData(folder+'/WT-14d_vs_WT-0d/DN_CC_level2_classFun.txt')
    bp=getLevelData(folder+'/WT-14d_vs_WT-0d/DN_BP_level2_classFun.txt')
    mf1=getLevelData(folder+'/WT-14d_vs_WT-0d/UN_MF_level2_classFun.txt')
    cc1=getLevelData(folder+'/WT-14d_vs_WT-0d/UN_CC_level2_classFun.txt')
    bp1=getLevelData(folder+'/WT-14d_vs_WT-0d/UN_BP_level2_classFun.txt')
    mf2=getLevelData(folder+'/WT-14d_vs_WT-0d/NU_MF_level2_classFun.txt')
    cc2=getLevelData(folder+'/WT-14d_vs_WT-0d/NU_CC_level2_classFun.txt')
    bp2=getLevelData(folder+'/WT-14d_vs_WT-0d/NU_BP_level2_classFun.txt')
    maxs=[len(getDataMatrix(folder+'/WT-14d_vs_WT-0d/DN.txt')[0]),len(getDataMatrix(folder+'/WT-14d_vs_WT-0d/UN.txt')[0]),len(getDataMatrix(folder+'/WT-14d_vs_WT-0d/NU.txt')[0])]
    wb=WegoBar({'BP':appendLevelData(appendLevelData(bp, bp1),bp2),'CC':appendLevelData(appendLevelData(cc, cc1),cc2),'MF':appendLevelData(appendLevelData(mf, mf1),mf2)},['DownNone','UN','NU'])
    wb.writeBar('DN_level2_classFun_Test3',folder+'/test.svg',maxs)
...
def test2():
    ...
    cc1=getLevelData(folder+'/WT-14d_vs_WT-0d/UN_CC_level2_classFun.txt')
    bp1=getLevelData(folder+'/WT-14d_vs_WT-0d/UN_BP_level2_classFun.txt')
    maxs=[len(getDataMatrix(folder+'/WT-14d_vs_WT-0d/DN.txt')[0]),len(getDataMatrix(folder+'/WT-14d_vs_WT-0d/UN.txt')[0])]
    wb=WegoBar({'BP':appendLevelData(bp, bp1),'CC':appendLevelData(cc, cc1),'MF':appendLevelData(mf, mf1)},['DN','UN'])
    wb.writeBar('DN_level2_classFun_Test2',folder+'/test.svg',maxs)
...
def test1():
    ...
    cc=getLevelData(folder+'/WT-14d_vs_WT-0d/DN_CC_level2_classFun.txt')
    bp=getLevelData(folder+'/WT-14d_vs_WT-0d/DN_BP_level2_classFun.txt')
    maxs=[len(getDataMatrix(folder+'/WT-14d_vs_WT-0d/DN.txt')[0])]
#     maxs=[123456]
    wb=WegoBar({'BP':bp,'CC':cc,'MF':mf},['DN'])
    wb.writeBar('DN_level2_classFun_Test1',folder+'/test.svg',maxs)
